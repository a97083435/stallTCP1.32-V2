...
- [🔑 snippets专用用户配置区域说明（重要 - 必读）](#-snippets专用用户配置区域说明重要---必读)
  ...
  - [常见特征词拆分参考](#常见特征词拆分参考)
  - [哪些变量可以留空？](#哪些变量可以留空)
  - [变量名与环境变量对照表（仅 Worker 版适用）](#变量名与环境变量对照表)
- [代码版本说明](#-代码版本说明)
- [界面预览](#️-界面预览)
- [懒人使用指南](#-懒人使用指南)
  - [图文教程](#-图文教程)
...
- [Cloudflare 统计功能说明](#-cloudflare-统计功能说明)
  ...
  - [后台显示逻辑](#后台显示逻辑)
- [CF 用量实时监控（Telegram）](#-cf-用量实时监控telegram)
- [访问日志与统计详解](#-访问日志与统计详解)
  - [访问日志功能](#访问日志功能)
  - [每日统计功能](#每日统计功能)
...
| `WL_IP` | **静态白名单 IP** (免检，视为管理员)<br>**✅ 支持多个，逗号 / 空格 / 换行 分隔（中英文符号不区分）** | `1.2.3.4` 或<br>`1.1.1.1,2.2.2.2` | ✅ |

### 🌍 节点来源配置

| 变量名 | 说明 | 格式说明 | 支持多值 |
| :--- | :--- | :--- | :---: |
| `ADD` | **本地优选 IP 列表** | 格式：`IP:端口#节点名称`<br>示例：`1.1.1.1:443#美国节点`<br>**✅ 换行 / 空格 / 逗号 三种分隔任选，中英文符号不区分** | ✅ (换行 / 空格 / 逗号) |
| `ADDAPI` | **远程 TXT 优选列表** | 填入 URL，内容格式同 ADD<br>**✅ 支持多个 URL，换行 / 空格 / 逗号 三种分隔任选** | ✅ (换行 / 空格 / 逗号) |
| `ADDCSV` | **远程 CSV 优选列表** | 填入 URL，支持高级节点信息导入<br>**✅ 支持多个 URL，换行 / 空格 / 逗号 三种分隔任选**<br>*配合 `DLS` 变量可过滤低速节点* | ✅ (换行 / 空格 / 逗号) |
| `ADDSUB` | **汇聚订阅**（一个框混写五类内容） | 优选IP / 优选API / 机场订阅 / sub:// 生成器 / 现成节点，详见下方专章 | ✅ (换行 / 空格 / 逗号) |

### 🔗 ADDSUB 汇聚订阅

一个输入框内可混写以下 **五类**内容，系统按**内容特征**自动识别并分别处理。分隔方式同其他多值变量（换行 / 空格 / 逗号，中英文符号不区分）。

| 类型 | 写法示例 | 处理方式 |
|---|---|---|
| ① 优选域名 / IP | `www.visa.cn#优选域名`<br>`104.24.0.232:8443#优选IPv4`<br>`[2606:4700::]:2053#优选IPv6` | 套用本项目节点模板 |
| ② 优选 IP API | `https://raw.githubusercontent.com/xxx/best_ips.txt#优选v4` | 抓取后套用本项目节点模板 |
| ③ 机场订阅 | `https://69yun69.net/auth/register?code=X#69云机场` | **原样透传**，排在订阅最前 |
| ④ 优选订阅生成器 | `sub://sub.example.net#CM优选订阅` | 哨兵探测取地址，套用本项目节点模板 |
| ⑤ 现成节点链接 | `vl` + `ess://uuid@host:443?type=ws&host=...` | **原样透传**，排在订阅最前 |

**② 与 ③ 写法完全相同**，靠抓回来的内容自动区分：内容含 `://` 视为节点订阅（③，透传），否则视为 IP 列表（②，套模板）。

#### 处理规则

- **① ② ④ 得到的是地址**，会合并进 `ADD`/`ADDAPI`/`ADDCSV` 的优选池，统一套用本项目的节点模板（含 ECH、指纹、path 等配置）
- **③ ⑤ 是别人的完整节点**，原样透传不加工并排在订阅最前。不注入 ECH / 指纹 / PS 后缀 —— 外部节点有自己的 `host`/`sni`，强行注入会导致节点失效
- 每个条目后的 `#备注名` 会作为来源标签追加到该源所有节点名尾部，形如 `香港01 [69云机场]`，便于辨认来源
- 单个源抓取失败**静默跳过**，不影响其余源；单源超时 5 秒；所有源并行抓取
- 支持 UTF-8 / GBK 自动解码（国内机场订阅常见 GBK 输出），支持 base64 与明文订阅自动识别
- CSV 支持两种表头格式（`IP地址,端口,TLS,数据中心,国家` 与 `IP,延迟,下载速度`），靠表头名定位列
- 裸 IPv6（如 `2606:4700:3033::ac43:a1b1`）会自动规范成 `[2606:4700:3033::ac43:a1b1]:443`

#### `?proxyip=true` 语法糖

在任意 **②** 类条目后追加 `?proxyip=true`，该源的 IP 会同时作为对应节点的反代地址，节点 path 自动变为 `/proxyip=<自身地址>`：

```
https://example.com/ips.txt?proxyip=true#兼作反代
```
...
| Sing-box / Karing / Hiddify | ✅ | ✅ |
| Surge / Quantumult X / Loon | ✅ | ✅ |

> **实现说明**：走订阅转换的客户端（Clash / Sing-box / Surge / QuanX / Loon），③ ⑤ 通过 subconverter 的 `|` 多源参数传给转换后端；① ② ④ 由本项目直接生成节点。
> 若你的转换后端不支持 `|` 多源语法，③ ⑤ 在这些客户端上可能不可见，其余客户端不受影响。
> `url` 参数总长超过 6000 字符时会截断多余源，避免被转换后端拒绝。
...
[2606:4700::]:2053#优选IPv6
https://raw.githubusercontent.com/xxx/my-cf-ip/main/best_ips_iponly.txt#优选v4
https://raw.githubusercontent.com/xxx/my-cf-ip/main/best_ips_iponly_v6.txt#优选v6
https://69yun69.net/auth/register?code=RSuNkt#69云机场
sub://sub.example.net#CM优选订阅
```

在 Cloudflare 环境变量界面（只能单行）改用空格分隔即可，效果相同。
...
**功能：** 支持多个 IP 或多个远程 URL，所有节点会合并生成。

> **💡 分隔符说明（重要）**
>
> `ADD` / `ADDAPI` / `ADDCSV` 三项均支持 **换行**、**空格**、**逗号** 任意一种分隔，且**中英文符号不区分**：
>
> | 类型 | 可用符号 |
> | :--- | :--- |
> | 换行 | 回车换行 |
...
> | 逗号 | 英文 `,`、中文 `，`、顿号 `、` |
>
> **为什么需要这个：** Cloudflare 新版环境变量界面（Workers → 设置 → 变量）的输入框**只能单行输入，无法回车换行**，因此在 CF 后台填多个值时请改用**空格**或**逗号**分隔。在本项目后台管理页面里用换行分隔同样有效。
>
> **⚠️ 注意：** 备注名里含空格或逗号不会被误切（如 `1.2.3.4:443#美国 洛杉矶` 正常识别为一个节点）；分隔时只用**一个**符号即可，逗号后不要再加空格。
...
空格或逗号分隔（Cloudflare 后台单行输入）：
```
1.1.1.1:443#美国CF 8.8.8.8:443#谷歌DNS
1.1.1.1:443#美国CF,8.8.8.8:443#谷歌DNS
1.1.1.1:443#美国CF，8.8.8.8:443#谷歌DNS
```

**ADDAPI 格式：** URL 列表（换行 / 空格 / 逗号 分隔）
```
https://example.com/ips.txt
https://backup.com/nodes.txt
```
```
https://example.com/ips.txt https://backup.com/nodes.txt
```

**ADDCSV 格式：** URL 列表（换行 / 空格 / 逗号 分隔）
```
https://example.com/优选IP.csv
https://backup.com/speed-test.csv
```
```
https://example.com/优选IP.csv,https://backup.com/speed-test.csv
```
